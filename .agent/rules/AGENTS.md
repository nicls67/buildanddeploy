...
The repo contains the `BuildAndDeploy` Python script which purpose is to build and deploy apps automatically based on a YAML configuration file.

## Repo layout
1. `build.py` is the script entry point
2. `libs/` contains all functions used by the script
3. `templates/` contains templates of YAML configuration files.
...
- If you need to touch multiple files, explain why in the final response.
...
- Update the script version in file `version.py` after each change following SEMVER rules by comparison with the last git tag, version shall not be necessarily updated after each commit.

## Naming rules
1. Follow standard Python naming conventions
2. Use the following rule to name variables and constants : constants shall start with 'K', global variables shall start with 'G', local variables shall start with 'l', functions and methods parameters shall start with 'p'
...
Read the diff and focus primarily on:
  ...
  - Check the script version has been correctly updated (compare with previous tag)
...
- “Review this diff and run ruff/mypy/pytest”
- "@my_review"
